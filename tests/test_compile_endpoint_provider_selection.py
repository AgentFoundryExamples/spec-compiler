...
class TestParametrizedProviderSelection:
    ...
    def test_provider_creates_correct_client_type(
        self, provider: str, expected_client_type: str, mock_publisher: Mock
    ) -> None:
        ...
        with (
            ...
            if provider == "openai":
                with patch("spec_compiler.services.openai_responses.OpenAiResponsesClient") as mock_client_class:
                    mock_client = Mock()
                ...
                with patch("spec_compiler.services.anthropic_llm_client.ClaudeLlmClient") as mock_client_class:
                    mock_client = Mock()
                    mock_client_class.return_value = mock_client
                    mock_client.generate_response.return_value = LlmResponseEnvelope(
                        request_id="test-req",
